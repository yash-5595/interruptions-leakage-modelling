...
class AttentionDecoderCell(nn.Module):
    def __init__(self, hidden_size, sequence_len, out_size, spatial_inp_size = 7):
        super().__init__()
        # attention - inputs - (decoder_inputs, prev_hidden)
        self.attention_linear = nn.Linear(hidden_size + out_size, sequence_len)
        self.attention_spatial = nn.Linear( spatial_inp_size + hidden_size, hidden_size)
        self.attention_stich = nn.Linear(hidden_size + sequence_len, hidden_size)
    ...
    def forward(self, encoder_output, prev_hidden, y, sig_timing, index, sim_params):
            # 50x400x150 50x150 50x1 50x400x8 50x13
        # adding spatial attention below - inp - prev hidden, sig timing, sim_params

        # 50x8, 50x150, 50,13

        spatial_attention_input = torch.cat((sig_timing, prev_hidden, sim_params), axis=1)
#         logging.info(f" spatial_attention_input {spatial_attention_input.shape}")
        spatial_weights = F.softmax(self.attention_spatial(spatial_attention_input), dim=1).unsqueeze(1)

#         logging.info(f"spatial_weights{spatial_weights.shape}")
        ...
        spatial_combine = torch.bmm(spatial_weights,encoder_output.reshape(-1,encoder_output.shape[2],encoder_output.shape[1] ) ).squeeze(1)
#         print(f"spatial_combine shape {spatial_combine.shape} temporal combine shape {attention_combine.shape} ")
        attention_concat = torch.cat((attention_combine, spatial_combine), axis=1)
        attention_out = self.attention_stich(attention_concat)

#         print(f"attention combine shape {attention_combine.shape }")
        rnn_hidden = self.decoder_rnn_cell(attention_out, prev_hidden)
